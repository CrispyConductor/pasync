...
exports.otto = function(tasks) {
	var keys = Object.keys(tasks);
	keys.forEach(function(task) {
		var isArray = Array.isArray(tasks[task]);
		console.log(task + ' is array -- ' + isArray);
		if(isArray) {
			var promiseFn = tasks[task][tasks[task].length - 1];
			tasks[task] = constructIterator(promiseFn, 0, false, false);
		} else {
			var promiseFn = tasks[task];
			tasks[task] = constructIterator(promiseFn, 0, false, false);
		}
			console.log(promiseFn.toString());
	});
	return new Promise(function(resolve, reject) {
