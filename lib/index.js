...
// exports.during = convertTaskFunction(regAsync.during, 1, 0, 2, true);

//promiseIterator, iteratorCallbackPos, iteratorProvidesValue, noErrors, unhandledErrorHandler
exports.during = function() {
	var args = Array.prototype.slice.call(arguments, 0);
	var testFn = args.shift();
	var iteratorFn = args.shift();
	console.log(iteratorFn.toString());
	var newIteratorFn = constructIterator(iteratorFn, 0, false, false);
	console.log('about to go...');
	var self = this;
	return new Promise(function(resolve, reject) {
		regAsync.during.call(self, testFn, newIteratorFn, function(err) {
			console.log('ayyy');
			if(err) {
				reject(err);
			} else {
